...
const AlJazariyyahLessons = () => {
  ...
  const fetchLessons = async () => {
    try {
      const endpoint = `/lessons?alJazariyyahCourseId=${alJazariyyahCourseId}&page=${page}&size=${size}`
                ...
                lessons.map((lesson) => (
                  <TableRow key={lesson.id}>
                    ...
                    <TableCell className="text-center space-x-2 rtl:space-x-reverse">
                      <Link to={`/tajweed/alJazariyyah/${alJazariyyahCourseId}/lessons/${lesson.id}`}>
...
export default AlJazariyyahLessons
