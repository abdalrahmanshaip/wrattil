...
const TuhfatLessons = () => {
  ...
  const fetchLessons = async () => {
    try {
      const endpoint = `/lessons?tuhfatCourseId=${tuhfatCourseId}&page=${page}&size=${size}`
                ...
                lessons.map((lesson) => (
                  <TableRow key={lesson.id}>
                    ...
                    <TableCell className="text-center space-x-2 rtl:space-x-reverse">
                      <Link to={`/tajweed/tuhfat/${tuhfatCourseId}/lessons/${lesson.id}`}>
